	...
	const handleChangePassword = () => {
		try {
			...
			changePassword(payload).then(
				(res: apiResponse<ResetPasswordProfileResponse>) => {
					if (res.hasError) {
						setIsLoading(false);
						Toast.show({
							type: "error",
		...
		<View className="flex-1 bg-[#F5F7FF] space-y-4">
			...
			<View className="space-y-4 mx-4">
				<View className="h-[64px] bg-white rounded-md">
					<Text className="text-xs ml-4 mt-3 text-gray-400">
						Anterior contraseña
					</Text>
					<TextInput
						value={oldPassword}
			...
			<View className="h-[40%] flex flex-col justify-end mx-4">
				...
				>
					<Text className="text-[#4A44C6] font-bold text-lg">
						Guardar
					</Text>
				</Pressable>
			</View>
