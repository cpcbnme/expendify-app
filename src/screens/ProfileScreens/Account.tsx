			...
			saveUserProfilePicture(imageResult)
				.then((res: apiResponse<ProfilePictureResponse>) => {
					...
					Toast.show({
						type: "success",
						text1: "OK",
						text2: "Hemos actualizado su foto de perfil",
					});
					setIsLoading(false);
				})
				.catch((err) => {
					Toast.show({
						type: "error",
						text1: "Error",
						text2: "Ha ocurrido un error al actualizar su foto de perfil",
					});
					setIsLoading(false);
				});
		} else {
			...
			updateUserDetails(data)
				.then((res: apiResponse<string>) => {
					Toast.show({
						type: "success",
						text1: "OK",
						text2: "Hemos actualizado los detalles de su perfil",
					});
					setIsLoading(false);
				...
				.catch((err) => {
					Toast.show({
						type: "error",
						text1: "Error",
						text2: "Ha ocurrido un error al actualizar los detalles de su perfil",
					});
					setIsLoading(false);
				});
		}
	...
	return (
		<SafeAreaView className="mt-10 mx-4 space-y-10">
			<View className="w-[60%] flex flex-row justify-between items-center">
				...
				<Text className="font-bold text-lg">Mi cuenta</Text>
			</View>
			...
			<View className="space-y-3">
				<View className="flex-row justify-between items-center">
					...
					<View className="bg-white h-20 rounded-md justify-center pl-4 w-[48%]">
						<Text className="text-gray-400">Apellido</Text>
						<TextInput
							className="font-bold w-full"
							value={lastNameInput}
							editable={editProfile}
				...
				<View className="bg-white h-20 rounded-md justify-center pl-4">
					<Text className="text-gray-400">Correo electrónico</Text>
					<Text className="font-bold">{email}</Text>
				</View>
				<View className="bg-white h-20 rounded-md justify-center pl-4">
					<Text className="text-gray-400">Telefono</Text>
					<TextInput
						className="font-bold w-full"
			...
			<TouchableOpacity onPress={handleEditProfile}>
				...
				>
					...
					>
						{editProfile ? "Actualizar detalles" : "Editar perfil"}
					</Text>
				</View>
			</TouchableOpacity>
